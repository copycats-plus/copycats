...
public class CopycatHalfModel extends CopycatModel {
    ...
    protected List<BakedQuad> getCroppedQuads(BlockState state, Direction side, RandomSource rand, BlockState material,
        ...
        Axis axis = state.getOptionalValue(CopycatHalfBlock.AXIS).orElse(Axis.Y);
        ...
        for (boolean row : Iterate.trueAndFalse) {
            for (boolean column : Iterate.trueAndFalse) {
                ...
                Vec3 rowShift = rowNormal.scale(row ? -2 / 16.0 : 2 / 16.0);
                Vec3 columnShift = columnNormal.scale(column ? -2 / 16.0 : 2 / 16.0);
