...
/* Copied from Steam 'N' Rails github as they were the only ones i coud find with a good multiloader packet implementation*/
/**
...
public abstract class PacketSystem {
    ...
    protected PacketSystem(String id, int version,
    ...
    public void handleS2CPacket(Minecraft mc, FriendlyByteBuf buf) {
        ...
        if (i < 0 || i >= s2cPackets.size()) {
            Copycats.LOGGER.error("Invalid S2C Packet {}, ignoring", i);
            return;
        }
    ...
    public void handleC2SPacket(ServerPlayer sender, FriendlyByteBuf buf) {
        ...
        if (i < 0 || i >= c2sPackets.size()) {
            Copycats.LOGGER.error("Invalid C2S Packet {}, ignoring", i);
            return;
        }
